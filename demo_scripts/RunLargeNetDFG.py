...
goldnet = Network()
goldnet.read_goldstd("datasets/Large_Network/Ecoli-1_goldstandard.tsv")
...
settings["global"]["time_series_files"] = "datasets/Large_Network/Ecoli-1_dream4_timeseries.tsv"
...
for p in grid:
    ...
    dfg.setup(timeseries_storage, trans_factors, settings, "dfg4grn-large-net-test_ETAZ-" + str(p[0]) + "_LW-"+str(p[1])+"_TAU-"+str(p[2]), 1)
    jobman.queueJob(dfg)
...
settings["dfg4grn"]["lambda_w"] = 0.01
settings["dfg4grn"]["tau"] = 3
dfg.setup(timeseries_storage, trans_factors, settings, "dfg4grn-large-net-test_custom")
jobman.queueJob(dfg)
